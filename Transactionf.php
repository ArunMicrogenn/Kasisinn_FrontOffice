...
function __construct() {
	...
	public function ResCheckinsave() 
	{
		...
		if($numberofrow ==0)
		{
		 $qry= " Exec_RuningCustomer '".$_REQUEST['Firstname']."','".$_REQUEST['Lastname']."','".$_REQUEST['Title']."','".$_REQUEST['Mobile']."','".$_REQUEST['Email_ID']."','".$_REQUEST['City_id']."','".$_REQUEST['State_id']."','".$_REQUEST['Country_id']."','".$_REQUEST['Middlename']."'";
		...
		$sql = $this->db->query($delete);
			...
			$ins=$ins."Insert into Trans_Checkin_mas (Grcno,CheckinDate,CheckinTime,ExpDate,ExpTime,Noadults,Nochilds,NoofRooms,userid)
			values(dbo.GrcNo(),'".$Indate."',convert(VARCHAR,getdate(),108),'".$todate."',convert(VARCHAR,getdate(),108),'".$_REQUEST['Adults'][$i]."','".$_REQUEST['Child'][$i]."','1','".User_id."')";
	
			$ins=$ins."update Trans_Reserve_det set stat='Y', checkinrooms='1'
		...
		{   $sql7 = '';
			...
			echo "Sucess";
			if($res7){
				header("Location: http://192.169.178.237/frontoffice/index.php/Transaction/RoomStatusOnline");
			}
  ...
  $fromdate=$Indate;
	...
	{
		echo$sql="select rm.Room_Id, rm.RoomNo from Mas_RoomType rt
		inner join Mas_Room rm on rm.RoomType_Id=rt.RoomType_Id
		inner join room_status rs on rm.room_id = rs.Roomid
		where  rs.status<>'Y' and rs.notready<>1 and rs.foblock<>1 
		and rs.mblock<>1 and rs.billsettle <>1 and  rt.RoomType_Id='".$_REQUEST['type']."'";
        $res=$this->db->query($sql);
	...
	{	 
	  ...
	  $ins2='';
	  $ins3='';
	  $ins="Insert into Trans_reserve_mas(ResNo,Reserveby,Communicate,Resdate,cusid,totrooms,stat,userid,waitlist,duedate,travelagentid,onlinebookingno,age,discper,discount,muserid,company)
	  values('".$_REQUEST['resno']."','".User_id."','P','".$date."','".$Customer_Id."','".$_REQUEST['roomcount']."','','".User_id."','".$_REQUEST['ReservationMode']."','".$date."','".$_REQUEST['travelagent_Id']."','".$_REQUEST['bookingid']."','','".$_REQUEST['discper']."','".$_REQUEST['discamt']."','".User_id."',
	   '".$_REQUEST['CompanyId']."')";
