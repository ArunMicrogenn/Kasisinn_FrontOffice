...
function __construct() {
	...
	{
		$boolen=true;
		$opening = 0;$closing=0;$curClosing=0;
		 
		 $nextYear = date('Y');
		 $curDate = date('d');
		 $curMonth = date('m');
		 $curDate = settype($curDate, 'int');
		 $curMonth = settype($curMonth, 'int');
	...
	{ 
	 ...
	 $sql=" Select * from Mas_Room Rm
		...
		foreach ($res->result_array() as $row)
		{
			   ...
			   if(!file_exists($path)){
				...
				mkdir($path, 0777,true);   
				 ...
				 if(!file_exists($desdirc)){
					 move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $desdirc);
				 }ELSE{
					 move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $desdirc);
				 }
				}
			...
			WorkAddress2='".$_REQUEST['workaddress2']."',WorkAddress3='".$_REQUEST['workaddress3']."', Photopath = '".$newname."'
	...
	public function ResCheckinsave() 
	{
		...
		{   $sql7 = '';
			...
			echo "Sucess";
			if($res7){
				header("Location: http://localhost/ho/index.php/Transaction/RoomStatusOnline");
			}
	...
	public function Checkinsave()
	{
			...
			foreach ($res->result_array() as $rows)
		{
		  ...
		  $extension = explode(".", $_FILES["fileToUpload"]["name"] );
		  $newname   = ''.$_REQUEST['Mobile']."." . $extension[1];
		  $desdirc = "$path/$newname"; 
		  if(!file_exists($path)){
	  ...
	  if($numberofrow ==0)
	  {
		$qry= " exec Exec_RuningCustomercheckin '".$_REQUEST['Firstname']."','".$_REQUEST['Lastname']."','".$_REQUEST['Title']."','".$_REQUEST['Mobile']."','".$_REQUEST['Email_ID']."','".$_REQUEST['City_id']."','".$_REQUEST['State_id']."','".$_REQUEST['Country_id']."','".$_REQUEST['Middlename']."','".@$_REQUEST['Company_Id']."','SAVE', '".$newname."'";
		$resq=$this->db->query($qry);
		$this->db->close();
		$this->db->reconnect();	
      }
	  ...
	  {
		$qry= " exec Exec_RuningCustomercheckin '".$_REQUEST['Firstname']."','".$_REQUEST['Lastname']."','".$_REQUEST['Title']."','".$_REQUEST['Mobile']."','".$_REQUEST['Email_ID']."','".$_REQUEST['City_id']."','".$_REQUEST['State_id']."','".$_REQUEST['Country_id']."','".$_REQUEST['Middlename']."','".@$_REQUEST['Company_Id']."','UPDATE','".$newname."'";
		$resq=$this->db->query($qry);
		$this->db->close();
		$this->db->reconnect();
	  }
...
public function ResRoomCancelList($ID=-1,$BUT='SAVE')
	...
	public function ReinstateCheckoutSave(){
		...
		$sql = "update trans_checkout_mas set cancelflag='1' , reason = '".$reason."' , canceluserid='".User_id."' where checkoutid='".$checkoutid."'";
		$this->db->query($sql);
		$sql1 = "update room_status set billsettle='0' where roomgrcid='".$_REQUEST['roomgrcid']."' and grcid='".$_REQUEST['grcid']."'";
		$res = $this->db->query($sql1);
		if($res){
			echo "Success";
		}else{
	...
	public function ReinstateSettlementSave(){
		...
		if($no == 0){
			...
			$sql = "update trans_checkout_mas set cancelflag='1' , reason = '".$reason."', canceluserid='".User_id."',reinstate='1' , Settle = '0', reinstatedate='".date('Y-m-d')."' where checkoutid='".$checkoutid."'";
			$this->db->query($sql);
