...
function __construct() {
	...
	{
		$this->load->model('Transaction/CheckoutResettlement');
		$this->CheckoutResettlement->CheckoutResettlement_Val();
	}
	...
	public function Collection_Val()
	{
       ...
       $ins="";$ins1="";$ins3=""; $ims4="";
		...
		// $this->load->model('Transaction/Collection');
		// $this->Collection->Collection_Val();
	...
	Public function extendoverstay()
	{
		...
		$sql=" select det.grcid,* from Room_Status Rs
		 ...
		 where Rs.Status='Y' and det.depdate='".$date."'";
	...
	{
		echo $sql=" Select  mt.Title+'.'+cus.Firstname as Name, isnull(sum(amas.amount),0) as Amount,rs.roomgrcid from Mas_Room Rm
	...
	{ 
	 ...
	 $ins="insert into trans_reservecancel_mas (resno,reserveid,communicate,address1,resdate,restime,address2,advamount,payid,remarks,cancelorextend,refund,totrooms,conference,userid,chqno,bankid,duedatetypeid)
	 values(dbo.ResclNo(),'".$_REQUEST['Roomid']."',0,'".$Address1."',convert(varchar(25),getdate(),101),convert(varchar(25),getdate(),108),'".$Address2."','".$Amount."','0','".@$_REQUEST['remark']."','C','','1','0','".User_id."','0','0','0')";
	...
	{ 
		...
		$date=date("Y-m-d");
		$time= date("H:i:s");
		$ins1="Insert into Trans_credit_entry (CreditNo,Roomid,Grcid,CreditDate,Creditheadid,Amount,roomgrcid,payid,Bankid,otherAmount,UserID,tarrifftype,tarriffsetupid)
		values(dbo.Credit_Entry_No(),'".$_REQUEST['Roomid']."','".$_REQUEST['grcid']."','".$date."','".$_REQUEST['Revenue_Id']."','".$_REQUEST['Postamt']."','".$_REQUEST['roomgrcid']."','0','0','".$_REQUEST['Postamt']."','".User_id."','0','0')";
	...
	{ 
	  ...
	  $ins="Insert into Trans_receipt_mas (Receiptno,ReceiptType,Amount,paymodeid,cancel,grcid,roomgrcid,rptdate,roomid,narration,customerid,bank,cardnumber,validdate,ADJUSTAMOUT,userid,rpttime,billremark)
	  values(dbo.Advancereceiptno(),'C','".$_REQUEST['givenadvance']."','".$_REQUEST['paymode']."','0','".$_REQUEST['grcid']."','".$_REQUEST['roomgrcid']."','".$date."','".$_REQUEST['Roomid']."','".$narration."','".$_REQUEST['cusid']."','".$bank."','".$cardnumber."','".@$_REQUEST['validate']."','".$_REQUEST['givenadvance']."','".User_id."','".$time."','".$_REQUEST['advanceremark']."')";
      $ins1="Insert into Trans_Advancereceipt_mas (receiptid,amount,roomgrcid,type,adjustamount,userid)
	  values(@siden,'".$_REQUEST['givenadvance']."','".$_REQUEST['roomgrcid']."','RMS','".$_REQUEST['givenadvance']."','".User_id."')";
	 ...
	 if($numberofrow ==0)
	  {
		...
		'".$_REQUEST['documenttype']."','".$_REQUEST['documentno']."')";
	  ...
	  else
	  {
		...
		VISA_Issueplace='".$_REQUEST['visaissueplace']."',VISA_Issuedate='".$_REQUEST['visadateofissue']."',VISA_Expirydate='".$_REQUEST['visaexpirydate']."',Id_Documenttype='".$_REQUEST['documenttype']."',Id_Documentno='".$_REQUEST['documentno']."' where Mobile ='".$_REQUEST['gmobile']."'";  
	    $res=$this->db->query($up);
	  }
	...
	public function ResCheckinsave()
	{
	  ...
	  $ins5="update Trans_Reserve_Mas set stat='Y' where ResNo='".$_REQUEST['ResNo']."' ";
	  $ins6="update Trans_Reserve_det set stat='Y', checkinrooms='1'
	  ...
	  $ins1=$ins1."INSERT INTO Trans_Roomdet_det (resno,grcid,Roomid,Noofpersons,roomgrcid,typeid,roomrent,depdate,deptime,guestcharge,Extrabed,tarrifftypeid,ratetypeid,Actroomrent,userid,Actrackrate,taxid,companyid,travelagentid,onlinebookingno,discper,discamount,planid,plancharges,male)
			values('".$_REQUEST['ResNo']."',@Siden,'".$_REQUEST['Roomid']."','".$_REQUEST['Adults']."',
			'".$roomgrcid."','".$RoomType_Id."','".$RoomRent."','".$todate."',convert(VARCHAR,getdate(),108),
			'".$guestcharge."','".$Extrabed."','".$RoomType_Id."','".$_REQUEST['RateCode']."',
			'".$RoomRent."','".User_id."','".$actRoomRent."','".$Taxid."','".$Company_Id."',
			'".$_REQUEST['travelagent_Id']."','".$_REQUEST['bookingid']."','".$_REQUEST['discper']."','".$_REQUEST['discamt']."','".$_REQUEST['foodplan']."','0','".$_REQUEST['Adults']."')";		
	...
	$sql6="select * from temp_trans_roomdet_det_rent where roomid='".$_REQUEST['Roomid']."'";
	   ...
	   echo "BEGIN Try ";
				...
				{   
					...
					$date = date('Y-m-d', strtotime($_REQUEST['Indate']));
					$sql7=" exec Exec_Temp_Trans_credit_entry   '".$_REQUEST['Roomid']."', '".$date."'";
					$res7=$this->db->query($sql7);
					echo "Sucess";
					if($res7){
						header("Location: RoomStatusOnline");
					}
	...
	public function Checkinsave()
	{
	  ...
	  if($numberofrow ==0)
	  {
		$qry= " Exec_RuningCustomer '".$_REQUEST['Firstname']."','".$_REQUEST['Lastname']."','".$_REQUEST['Title']."','".$_REQUEST['Mobile']."','".$_REQUEST['Email_ID']."','".$_REQUEST['City_id']."','".$_REQUEST['State_id']."','".$_REQUEST['Country_id']."','".$_REQUEST['Middlename']."','".@$_REQUEST['Company_Id']."','SAVE'";
		$resq=$this->db->query($qry);
		$this->db->close();
		$this->db->reconnect();	
      }
	  ...
	  {
		$qry= " Exec_RuningCustomer '".$_REQUEST['Firstname']."','".$_REQUEST['Lastname']."','".$_REQUEST['Title']."','".$_REQUEST['Mobile']."','".$_REQUEST['Email_ID']."','".$_REQUEST['City_id']."','".$_REQUEST['State_id']."','".$_REQUEST['Country_id']."','".$_REQUEST['Middlename']."','".@$_REQUEST['Company_Id']."','UPDATE'";
		$resq=$this->db->query($qry);
		$this->db->close();
		$this->db->reconnect();
	  }
	...
	$sql6="select * from temp_trans_roomdet_det_rent where roomid='".$_REQUEST['Roomid']."'";
	   ...
	   echo "BEGIN Try ";
				...
				if($result) 
				{
					...
					$qry = "select mh.Company as Com, mc.Company as travelagent,* from Trans_checkin_mas ch 
					inner join Trans_RoomCustomer_det det on det.grcid = ch.Grcid
					inner join mas_customer cus on cus.Customer_Id = det.Customerid
					inner join mas_title t on t.Titleid = cus.Titelid
					inner join Trans_Roomdet_det roomdet on roomdet.grcid=ch.Grcid
					...
					$temqury = "select Template from mas_smsmessage where  isnull(CheckinGust,0)='1'";
					$smsTemp = $this->db->query($temqury);
					foreach($smsTemp->result_array() as $row){
						 $msg = $row['Template'];
					...
					$temquy = "select Template from mas_smsmessage where  isnull(CheckinMD,0)='1'";
					$smTemp = $this->db->query($temquy);
					foreach($smTemp->result_array() as $ro){
						 $msgg = $ro['Template'];
						 $msgg = str_replace('*GuestName*', $name, $msgg);
						 $msgg = str_replace('*RoomNo*',$roomno, $msgg);
						 $msgg = str_replace('*RoomType*', $roomType, $msgg);
						 $msgg = str_replace('*RoomRent*', $rent, $msgg);
	...
	{	 
	  ...
	  $ins="Insert into Trans_reserve_mas(ResNo,Reserveby,Communicate,Resdate,cusid,totamt,totrooms,stat,userid,waitlist,duedate,travelagentid,onlinebookingno,totalrent,age,discper,discount,muserid,Roomid)
	       values(dbo.ResNo(),'".User_id."','P','".$date."','".$Customer_Id."','".$RoomRent."','1','','".User_id."','0','".$date."','".$_REQUEST['travelagent_Id']."','".$_REQUEST['bookingid']."','".$RoomRent."','','".$_REQUEST['discper']."','".$_REQUEST['discamt']."','".User_id."','".$_REQUEST['Roomid']."')";
	 //$ins="Insert into Trans_Checkin_mas (Grcno,CheckinDate,CheckinTime,ExpDate,ExpTime,Noadults,Nochilds,NoofRooms,userid)
		//	values(dbo.GrcNo(),'".$Indate."',convert(VARCHAR,getdate(),108),'".$todate."',convert(VARCHAR,getdate(),108),'".$_REQUEST['Adults']."','".$_REQUEST['Child']."','1','".User_id."')";
  ...
  $fromdate=$Indate;
	...
	Public function roomtypegetroomnumber()
	{
	    ...
	    {
			echo "<option values=".$rows['Room_Id'].">".$rows['RoomNo']."</option>";
		}		
	}
	...
	Public function roomtypegettariff()
	{
		...
		Inner join Mas_RatePlan RP on RP.RoomType_Id=rt.RoomType_Id
		Inner join Mas_PlanType PT on PT.PlanType_Id=RP.PlanType_Id
		Inner join Mas_RatePlan_Det RPD on RPD.RatePlanDet_Id=RP.RatePlan_Id
		where rt.RoomType_Id=".$_REQUEST['type'];
        $res=$this->db->query($sql);
	    foreach ($res->result_array() as $rows)
	    { 
	...
	public function rackRate(){
		...
		foreach($ex->result_array() as $roww){
			...
			if ($_REQUEST['pax'] ==1){
				$data['Actrackrate']= $roww['Single'];
				$data['Extrabedamount']= 0.00;
				$roww_set[] = $data;
			}else if($_REQUEST['pax']==2){
				$data['Actrackrate']= $roww['Doubles'];
				$data['Extrabedamount']= 0.00;
				$roww_set[] = $data;
			}
			else if($_REQUEST['pax']==3){
				$data['Actrackrate']= $roww['Triple'];
				$data['Extrabedamount']= 0.00;
				$roww_set[] = $data;
			}
			else if($_REQUEST['pax'] >=4)
			{
				if($roww['Quadruple']==0)
				{
					$data['Actrackrate'] = $roww['Triple'] + $roww['Extrabedamount'];
					$data['Extrabedamount'] = $roww['Extrabedamount'];
					$roww_set[] = $data;
				}
				else
				{
					$data['Actrackrate'] = $roww['Quadruple'];
				    $roww_set[] = $data;
				}
			}	
	...
	public function UpiOptions(){
		...
		?>
		<select name="bank_1" class="m-ctrl" id="bank_1"><option value="">--Bank--</option>
		<?php 
	...
	public function UpiOptionSettle(){
		...
		$ex = $this->db->query($sql);
		?>
		<select name="bank_1" class="scs-ctrl" id="bank_1"><option value="">--Bank--</option>
		<?php 
		foreach($ex->result_array() as $row) 
		{
	...
	public function CompanyModeSettle(){
		...
		$ex = $this->db->query($sql);
		?>
		<select name="bank_1" class="scs-ctrl" id="bank_1"><option value="">--Company--</option>
		<?php 
		foreach($ex->result_array() as $row) 
		{
			?> 
	...
	public function GetReservationGuestDetails()
	{
		...
		Group By  mt.Title+'.'+cus.Firstname,rtmas.Amount ";
		$res=$this->db->query($sql);
	    foreach($res->result() as $row)
	    {  
			$data['Name']=$row->Name;
	...
	{	 
	  ...
	  if($numberofrow ==0)
	  {
	   $qry= " Exec_RuningCustomer '".$_REQUEST['Firstname']."','1','".$_REQUEST['Mobile']."','".$_REQUEST['Email_ID']."','".$_REQUEST['City_id']."'";
		$resq=$this->db->query($qry);
		$this->db->close();
		$this->db->reconnect();	
      }
	...
	//   $res8=$this->db->query($sql8);  
      ...
      $sql3="SELECT * FROM Mas_Room rm
		Inner join Mas_Roomtype rt on rt.RoomType_Id=rm.RoomType_Id
		where rm.Room_id='".$_REQUEST['Roomtype_id']."'";
	  $res3=$this->db->query($sql3);
	  foreach ($res3->result_array() as $row3)
	  {
		 $Room_id=$row3['Room_Id'];
	 ...
	 $sql4="SELECT * FROM temp_trans_roomdet_det_rent where roomid='". $Room_id."'";
	  ...
	  $ins="Insert into Trans_reserve_mas(ResNo,Reserveby,Communicate,Resdate,cusid,totamt,totrooms,stat,userid,waitlist,duedate,travelagentid,onlinebookingno,totalrent,age,discper,discount,muserid,Roomid)
	       values(dbo.ResNo(),'".User_id."','P','".$date."','".$Customer_Id."','".$RoomRent."','1','','".User_id."','0','".$date."','".$_REQUEST['travelagent_Id']."','".$_REQUEST['bookingid']."','".$RoomRent."','','".$_REQUEST['discper']."','".$_REQUEST['discamt']."','".User_id."','". $Room_id."')";
	...
	$sql6="select * from temp_trans_roomdet_det_rent where roomid='". $Room_id."'";
	   ...
	   echo "BEGIN Try ";
				...
				?>
				<script>
				alert("Reservation Save Successfully...!");
					windows.location.href="<?php echo scs_index?>Transaction/Reservation_";
