   ...
   <fieldset>
	  ...
	  $res=$this->db->query($qry);
       ...
       $sql2= "select * from Trans_credit_entry tcc
        ...
        Inner join Room_Status Rms on Rms.Roomid=tcc.Roomid
        where CreditDate='".$auditdate."' and ror.RevenueHead='ROOM RENT'
        and Rms.Status='Y' and tcc.roomgrcid not in (select Roomgrcid from Trans_checkout_mas) ";
      ...
      if($norow1 !=0)
		...
		<tr>
            <td align="right">
			<?php if ($bool==true){ ?><input type="button"   class="btn btn-success btn-sm" id="EXEC" name="EXEC"  value="<?php echo $BUT;?>"   /><?php } ?></td>
           
		    <td align="left"><!--<?php if ($norow1 !=0){ ?>
			<input type="button"   class="btn btn-success btn-sm" id="EXEC" name="EXEC"  value="Delete Posted Rent"   /><?php } ?> ---> <?php 
			if ($bool==false) { echo '<span style="color: red;">*Tariff post option valibale 23:00 after</span>';}?></td--->
		</tr>
      </table>
    </fieldset>
   </div>
