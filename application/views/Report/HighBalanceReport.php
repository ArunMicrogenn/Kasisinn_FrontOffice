...
<div id="printing" class="col-sm-12">
        ...
        <table class="table table-bordered table-hover"  >   
		    <div>
				<h3 class="text-center">High Balance Report  <h3>
		    </div>       
		   <tbody>
		    <?php 
			 $i=1;
		...
		left outer join mas_company cp on cp.Company = cmas.company  ORDER BY billamount desc";
			 ...
			 $advance= $exec->num_rows();
			  ...
			  {				
				...
				echo '<td style="text-align: center;">'.number_format($rows['totalbillamount'],2).'</td>';
				echo '<td style="text-align: right;">'.$rows['advance'].'</td>';
				echo '<td style="text-align: right;">'.number_format($rows['totalbillamount']-$rows['advance'],2).'</td>';
				echo '</tr>';				
			  }			 
		   ?>		   
		   </tbody>
