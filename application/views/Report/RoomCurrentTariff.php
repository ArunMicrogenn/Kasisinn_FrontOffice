	...
	<form action="" method="POST">
      <table class="FrmTable T-6" >
        <tr>
          ...
          <td align="left">
		  <input type="date" value="<?php if(@$_POST['frmdate'] ==''){ echo date('d-m-Y');}else{ echo $_POST['frmdate']; } ?>" id="frmdate" name="frmdate"   class="scs-ctrl" />
            <div class="Type" ></div></td>
			<td></td>
            <!--td align="right" class="F_val">To Date</td>
          <td align="left"><input type="text" value="<?php echo date('d-m-Y'); ?>" id="Type" name="Type"   class="scs-ctrl Dat" />
            <div class="Type" ></div></td--->
...
?>
        ...
        <table class="table table-bordered table-hover "  >
			<div>
				<h3 class="text-center">Room Current Tariff  <?php echo date('d-m-Y', strtotime($_POST['frmdate'])); ?><h3>
		    </div>
			<thead>
		   ...
		   <tbody>
		    <?php 
			 ...
			 $norow= $res->num_rows(); 
			  ...
			  {
				echo '<tr>';
				echo '<td colspan="11" class="text-bold" style="text-align: left;">In Houese Rooms</td>';
				echo '</tr>';
			  }
			 ...
			 echo '<td style="text-align: right;">'.number_format($row['Actrackrate'],2).'</td>';
			 echo '<td style="text-align: right;">'.number_format($row['roomrent'],2).'</td>';
			 ...
			 echo '</tr>';
			 $rackrate=$rackrate+$row['Actrackrate'];
			 $chargetariff=$chargetariff+$row['roomrent']; $cgsttol=$cgsttol+$cgst; $sgsttol=$sgsttol+$sgst;
		   ...
		   <?php 
		   $rackrate=0; $chargetariff=0; $cgsttol=0;$sgsttol=0; $grtotal=0; $guestchargetotal=0;
			$sql=" Select tcc.Amount as amt,tcc.Grcid,* from Trans_credit_entry tcc
			  Inner Join Mas_Revenue ror on ror.Revenue_Id=tcc.Creditheadid 
			  Inner join Mas_Room rm on rm.Room_Id=tcc.Roomid
			  Inner join Room_Status Rms on Rms.Roomid=rm.Room_Id
			  Inner join Mas_Roomtype typ on typ.RoomType_Id=rm.RoomType_Id 
			 ...
			 echo '<td style="text-align: left;">'.$row['Title'].'.'.$row['Firstname'].'</td>';
			 echo '<td style="text-align: center;">'.$row['Noofpersons'].'</td>';
			 echo '<td style="text-align: right;">'.$row['roomrent'].'</td>';
			 echo '<td style="text-align: right;">'.$row['amt'].'</td>';
			 echo '<td style="text-align: right;">'.number_format($row['guestcharge'],2).'</td>';
			 echo '<td style="text-align: right;">'.number_format($cgst,2).'</td>';
			 echo '<td style="text-align: right;">'.number_format($sgst,2).'</td>';
			 echo '<td style="text-align: right;">'.number_format($total,2).'</td>';
			 echo '</tr>';
			 $rackrate=$rackrate+$row['roomrent'];
			 $chargetariff=$chargetariff+$row['amt']; $cgsttol=$cgsttol+$cgst; $sgsttol=$sgsttol+$sgst;
			 $grtotal=$grtotal+$total; $guestchargetotal=$guestchargetotal+$row['guestcharge'];
			 $i++;
			 }
			  if($norow !=0)
